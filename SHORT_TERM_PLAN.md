...
2. Reuse the existing toy math tools as the first tools available to a test subagent.
3. Add a new `subtract()` `@tool` and include it in the test subagent.
4. Build a utility function that creates ReAct agents from a `Subagent`.
5. Build a dictionary of subagents in the form `{name: compiled_agent}` using that utility.
